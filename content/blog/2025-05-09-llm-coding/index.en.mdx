...
model: GPT-5.2-Codex
description: Reflecting on how using Cursor to improve this blog's code changed my perception of how practical LLM-assisted coding has become.
---

<Message title="Note" variant="warning" defaultOpen={true}>
  This article is based on information **as of 2025/05/01**. Information about
  LLMs changes daily, so please verify the latest details yourself.
</Message>
...
This time I used [Cursor](https://www.cursor.com/ja) to develop this blog. I built the blog around July last year, and after implementing the bare minimum (list/detail), I barely touched it. There were features I knew I should implement, but they were not essential, so I kept postponing them. Using Cursor, I managed to add a decent number of features during Golden Week.
...
This is likely partly because I didn't use [Rules](https://docs.cursor.com/context/rules), so there's room to improve. However, judging from the current pace, within six months we'll likely see models that surpass `claude-3.7-sonnet` and handle code context even more accurately.
...
How big is this blog's codebase now? Using [tokei](https://github.com/XAMPPRocky/tokei), I got:

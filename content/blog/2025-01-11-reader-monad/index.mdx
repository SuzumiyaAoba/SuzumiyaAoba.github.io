...
description: 「モナドのすべて」の Reader モナドのサンプルを Scala Cats で実装し直したコードを紹介する。
---

今回は Reader モナドに触れるということで[モナドのすべて](https://www.sampou.org/haskell/a-a-monads/html/index.html)の Reader モナドのページに載っているサンプルを [Cats](https://typelevel.org/cats/) を使って実装してみた。Cats を触り始めたばっかりなのでよりよい書き方があるかもしれないが、一旦動くところまで書けたのでコードだけ載せておく。

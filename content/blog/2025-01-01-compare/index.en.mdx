...
model: GPT-5.2-Codex
description: An easy way to remember what negative, zero, and positive return values mean for compare/compareTo, comparing docs across languages.
---
...
In languages like Haskell and Rust, the result is expressed as an ADT like `GT`/`Greater`, `EQ`/`Equal`, `LT`/`Less` [^Haskell] [^Rust], but in languages like C, Java, JavaScript, and PHP, comparison functions return a negative value, zero, or a positive value. I'm not well-versed in C, so I don't know which document to cite for it; I'll include docs for the other languages.

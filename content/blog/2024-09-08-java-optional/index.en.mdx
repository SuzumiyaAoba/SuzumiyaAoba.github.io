...
model: GPT-5.2-Codex
description: Framing Java's Optional as part of the lineage of Haskell's Maybe, and clarifying how to use it in line with its design intent.
---
...
We'll round down the tax because we're kind.[^tax-calc] If the format is neither `未定` nor `XXX円`, return `null`.
...
As an object, `Optional` is "a container that may or may not contain a non-null value." But if we return `Optional.empty()` on failure and `Optional.of(T)` on success, it becomes not just "some value or not", but a way to express success/failure.[^success-failure]
...
I hope Java will provide something like this in the standard library one day, but as of Java 21 it does not. The most practical option is to use a functional library like [vavr](https://github.com/vavr-io/vavr), but introducing such a library in production is hard.

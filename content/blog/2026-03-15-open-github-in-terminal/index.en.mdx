...
When working in the terminal, there are times when you want to open a GitHub URL immediately. In that case, if the repository is on **GitHub**, you can use the `gh` command ([GitHub CLI][GitHub CLI]) to open the URL directly in your browser.
...
I remembered that I could open a GitHub repository with the `gh` command, but I forgot which subcommand it was. So I searched with some likely keywords, but the simplest solution did not show up at the top.

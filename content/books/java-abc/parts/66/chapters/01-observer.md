...
[^java-flow-api]: Java SE 25 API, `java.util.concurrent.Flow`, <https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/concurrent/Flow.html>。Java 9（JEP 266）で導入された Reactive Streams 仕様（<https://www.reactive-streams.org/>）の Java 標準対応 API。`Publisher`／`Subscriber`／`Subscription`／`Processor` のインターフェースを定義する。

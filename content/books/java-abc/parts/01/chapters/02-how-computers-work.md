...
CPU が1秒間にどれだけの作業をこなせるかは、**クロック周波数**という値でおおまかに表されます。単位は **Hz**（ヘルツ）で、たとえば「3 GHz（ギガヘルツ）」の CPU は、1秒間におよそ30億回ものペースで動作します。
...
いまでは「コンピュータ」といえば機械を指しますが、もともと computer は「**計算する人**」を意味する言葉でした[^human-computer]。電子計算機が登場する前、天文学の計算やロケットの軌道計算などは、計算を専門に行う人々（その多くは女性でした）が、手作業で担っていたのです。彼女たちの職業名が、まさに「コンピュータ」でした[^nasa-human-computers]。やがて、その膨大な計算を機械が肩代わりするようになり、「コンピュータ」という呼び名も、人から機械へと受け継がれていきました。いま CPU が一瞬でこなしている計算は、かつて多くの人が、何日もかけて行っていた仕事なのです。
...
これは、書いている途中の文章が**メモリ（調理台）の上にしかなかった**ために起こります。保存という操作をして、はじめて文章は**ストレージ（冷蔵庫・倉庫）にしまわれ**、次に開いたときも残っているのです。
...
「1 TB（テラバイト）」のはずの外付けディスクをパソコンにつなぐと、容量が「931 GB」などと少なく表示されていた ―― そんな経験はありませんか。これは故障でも、だましでもありません。理由は、**「キロ・メガ・ギガ」の数え方が2通りある**ことにあります。ふだんの単位では「1キロ＝1000」ですが、コンピュータの内部は 0 と 1（2進数）で動くため、`2` を10回かけた **1024** を「1キロ」と数えるほうが自然なのです[^binary-prefixes]。ディスクを売る側は「1キロ＝1000」で容量を表示し、パソコン側は「1キロ＝1024」で計算することがあります。この小さな数え方の違いが積み重なって、表示される容量がずれて見えるのです。
...
[^human-computer]: Oxford English Dictionary, "computer, n." 初期の用例（17世紀以降）では「計算を行う人」を指す。Grier, David Alan, _When Computers Were Human_ (Princeton University Press, 2005) も歴史的経緯を詳述している。

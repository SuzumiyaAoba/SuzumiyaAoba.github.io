...
これにて Java 入門書のすべてが完了です。
プログラミングの基礎から、モダン Java、JVM の内側、そして設計の語彙となるデザインパターンまで、長い道のりを歩んでこられました。お疲れさまでした。

ここからは、ぜひ**実際のプロジェクト**で手を動かしてみてください。
本書で得た知識は、コードを書きながら少しずつ「自分のもの」になっていきます。
分からないことが出てきたら、本書に立ち戻ってもよいですし、公式ドキュメントや一次資料に当たる習慣を持っておくと、長く役に立ちます。

あなたの Java の旅が、ここから本格的に始まることを願っています。

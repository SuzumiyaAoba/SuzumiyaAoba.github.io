...
[^mockito]: Mockito Framework, <https://site.mockito.org/>。Java で広く使われるモッキングライブラリで、`mock()`／`when(...).thenReturn(...)`／`verify()` などの簡潔な API を提供する。Java SE と JUnit／TestNG 等のテストフレームワークと併用される。

...
たとえば、`UserRepository` インタフェースを実装した `FakeUserRepository` を、テスト用に手で書きます。
...
**Mockito** などのモックライブラリは、インタフェースから**自動で偽物のインスタンス**を作ってくれます。
...
## モックを使う前提 ― 「インタフェース越し」の依存

モックは、「**インタフェース越しに依存している**」と、ぐっと使いやすくなります。
...
ここでは、依存を**インタフェース型**で持っていて、**コンストラクタで受け取って**います。
このようなコードなら、テストでは「**インタフェースのモック**」をコンストラクタに渡すだけで、依存を差し替えられます。
...
- Mockito は、**インタフェースから動的にモックを作る**ライブラリです

...
export const Sheet1StackedChart: React.FC<Props> = ({ data }) => {
  ...
  const renderStackedChart = (
    ...
    stackedData.forEach((layer, i) => {
      ...
      g.append("path")
        .datum(layer)
        .attr("fill", `url(#pattern-${metricIndex % patterns.length}-${groupIndex})`)
    ...
    legend
      ...
      .attr("fill", (d) => {
        ...
        return `url(#pattern-${index % patterns.length}-${groupIndex})`;
      })
      .attr("stroke", (d) => {
        const index = availableMetrics.indexOf(d);
  ...
  useEffect(() => {
    if (svgRef1.current) {
      renderStackedChart(svgRef1.current, groups[0], 0);
    }
  }, [data]);

  useEffect(() => {
    if (svgRef2.current) {
      renderStackedChart(svgRef2.current, groups[1], 1);
    }
  }, [data]);

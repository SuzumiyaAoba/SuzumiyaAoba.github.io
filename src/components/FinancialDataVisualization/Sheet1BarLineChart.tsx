...
export const Sheet1BarLineChart: React.FC<Props> = ({ data }) => {
  ...
  useEffect(() => {
    if (svgRef1.current) {
      renderBarLineChart(svgRef1.current, groups[0], 0);
    }
  }, [data, selectedMetrics]);

  useEffect(() => {
    if (svgRef2.current) {
      renderBarLineChart(svgRef2.current, groups[1], 1);
    }
  }, [data, selectedMetrics]);
    ...
    <div className="my-8 space-y-8">
      ...
      <div>
        <div
          ...
          onClick={() => handleGroupClick(groups[0].metrics)}
        >
          {groups[0].name}
        </div>
          ...
          {groups[0].metrics.map((metric) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className="text-sm">{metric.split("|")[0].trim()}</span>
              </div>
            );
          })}
        </div>
      ...
      <div>
        <div
          className="text-center font-semibold text-sm mb-2 cursor-pointer hover:text-blue-600"
          onClick={() => handleGroupClick(groups[1].metrics)}
        >
          {groups[1].name}
        </div>
        <div className="overflow-x-auto">

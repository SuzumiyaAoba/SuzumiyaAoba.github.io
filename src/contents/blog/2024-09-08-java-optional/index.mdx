...
- 本記事か [Optionalクラスを意図されたとおりに使うための12のレシピ](https://blogs.oracle.com/otnjp/post/recipes-for-using-the-optional-class-as-its-meant-to-be-used-ja) を読もう
...
> null以外の値を含む場合と含まない場合があるコンテナ・オブジェクト。 
>
> ...
> 
> APIのノート:  
...
`Etiher` では `Right` に成功した場合の値 (*right* なので `Right` が正しい場合)、`Left` に失敗した場合の値を入れる。
...
- [OptionalがSerializableではない話と使い方まとめ - きしだのHatena](https://nowokay.hatenablog.com/entry/20150901/1441091495)
...
- [チームで取り組む！JavaでNullと真正面から向き合う #NULL - Qiita](https://qiita.com/hondaYoshitaka/items/db0aec0e051fdf73e448)

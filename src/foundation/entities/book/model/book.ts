...
export const getBookSlugs = cache(
  async (): Promise<string[]> => await listContentSlugs("books")
);

/** 書籍の概要を返す。index.md がなければ null。 */
export const getBookMeta = cache(
  async (bookSlug: string): Promise<BookMeta | null> => {
...
export const getBookToc = cache(
  async (bookSlug: string): Promise<BookChapter[]> => {
      ...
      chapters.map(async (chapter): Promise<BookChapter | null> => {
        ...
        const sections = await Promise.all(
          files.map(async ({ filename, section }): Promise<SectionRef> => {
            const parsed = await readBookSectionFile(
              bookSlug,
              chapter,
              filename
            );

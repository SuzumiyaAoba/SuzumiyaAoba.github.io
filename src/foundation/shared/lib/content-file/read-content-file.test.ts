...
describe("createArticleFileLister / readContentFileWithFallback", () => {
  beforeAll(async () => {
    ...
    await mkdir(path.join(contentRoot, "blog", "both-locales"), {
      recursive: true,
    });
    await writeFile(
      path.join(contentRoot, "blog", "both-locales", "index.md"),
      "ja content"
    );
    await writeFile(
      path.join(contentRoot, "blog", "both-locales", "index.en.md"),
      "en content"
    );

    await mkdir(path.join(contentRoot, "blog", "ja-only-mdx"), {
      recursive: true,
    });
    await writeFile(
      path.join(contentRoot, "blog", "ja-only-mdx", "index.mdx"),
      "ja mdx content"
    );

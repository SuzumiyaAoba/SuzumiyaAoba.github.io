...
export function Column({ title, children, className }: ColumnProps) {
  return (
    <aside
      className={cn("my-6 rounded-md bg-amber-50/60 px-4 py-3 dark:bg-amber-950/30", className)}
    >
      ...
      <div className="prose prose-sm mt-2 max-w-none font-serif">{children}</div>
    </aside>
  );
}

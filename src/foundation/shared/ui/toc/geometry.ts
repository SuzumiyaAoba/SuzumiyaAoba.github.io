...
export function findTocLink(container: HTMLElement, href: string): HTMLElement | undefined {
  return Array.from(container.querySelectorAll<HTMLElement>("a[href]")).find(
    (element) => element.getAttribute("href") === href,
  );
}

export function calcThumb(container: HTMLElement, active: string[]): TocThumbValues {
...
export function updateThumb(element: HTMLElement, [top, height]: TocThumbValues): void {

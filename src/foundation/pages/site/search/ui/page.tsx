...
export default function Page({ locale }: PageProps) {
  ...
  const pagePath = toLocalePath("/search", resolvedLocale);
  return (
      ...
      <main className="mx-auto flex-1 flex w-full max-w-6xl flex-col gap-8 px-4 pt-6 pb-10 sm:gap-10 sm:px-6 sm:pt-8 sm:pb-12">
        <section className="space-y-3">
          <h1 className="text-xs font-semibold uppercase tracking-[0.24em] text-muted-foreground">
            <I18nText locale={resolvedLocale} ja="検索" en="Search" />
          </h1>
        ...
        <Suspense fallback={<SearchLoading locale={resolvedLocale} />}>
          <SearchPanel locale={resolvedLocale} />
        </Suspense>
      </main>

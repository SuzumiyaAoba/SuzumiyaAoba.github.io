...
const dummyPosts: HomePageContentProps["latestPosts"] = titles.map(
  ([ja, en], index) => {
...
export const English: Story = {
  args: {
    ...
    series: series.map((item, index) => ({
      ...item,
      name:
        ["Design patterns", "Local AI agents", "Scala", "Nix"][index] ??
        item.name,
    })),
  },
  play: async ({ canvasElement }) => {
    ...
    const writing = within(
      main.getByRole("region", { name: "Latest articles" })
    );
    await expect(
      writing
        .getAllByRole("heading", { level: 3 })
    ...
    const topics = within(
      main.getByRole("navigation", { name: "Browse by tag" })
    );
...
export const TranslationFallback: Story = {
  ...
  play: async ({ canvasElement }) => {
    const writing = within(
      within(canvasElement).getByRole("region", { name: "Latest articles" })
    );
    await expect(
      writing
        .getAllByRole("heading", { level: 3 })
    ...
    await expect(
      writing.getByRole("link", { name: /日本語のみの記事/u })
    ).toHaveAttribute("href", "/en/blog/post/japanese-only/");
    await expect(
      writing.getByRole("link", { name: /English only/u })
    ).toHaveAttribute("href", "/en/blog/post/english-only/");
  },
};

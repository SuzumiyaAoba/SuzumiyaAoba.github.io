    ...
    <>
      {lane > 0 && (
        <span
          aria-hidden="true"
          className={cn(
            "absolute top-10 left-(--x) h-(--h) w-px opacity-40",
      ...
      <button
        ...
        className={cn(
          "group absolute top-(--y) left-(--x) z-10 flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border border-transparent bg-background/95 pr-1 transition-shadow hover:z-20 hover:border-border hover:shadow-sm focus-visible:z-20 focus-visible:outline-2 focus-visible:outline-ring",
          labelled
            ? "w-36 -translate-x-4"
            : "w-8 -translate-x-1/2 justify-center",
          selected && "border-ai-accent/40 shadow-sm"
        )}
        style={{
          "--x": `${position}%`,
          "--y": `${24 + lane * 40}px`,
      ...
      >
        <ProviderIcon
          ...
          className={cn(
            "size-7 rounded-lg",
            selected && "ring-2 ring-ai-accent"
          )}
        />
        {labelled && (
          <span

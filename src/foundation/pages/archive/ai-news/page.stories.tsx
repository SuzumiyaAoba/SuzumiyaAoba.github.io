...
async function chooseCalendarDate(canvasElement: HTMLElement, date: string) {
  ...
  const picker = within(
    document.getByRole("dialog", { name: "移動する日付を選択" })
  );
...
export const Japanese: Story = {
  play: async ({ canvasElement }) => {
    ...
    const document = within(canvasElement.ownerDocument.body);
    await expect(
      canvas.getByRole("heading", { name: "全期間のリリース間隔" })
    ).toBeVisible();
    await expect(
      canvas.getByRole("button", { name: "全期間の比較" })
    ...
    await userEvent.click(
      canvas.getByRole("button", { name: /GPT · 2026年3月5日/u })
    );
    await expect(
      document.getByRole("article", { name: "GPT Next" })
    ).toHaveTextContent("前回から 63 日");
    await userEvent.click(canvas.getByRole("button", { name: "カレンダー" }));
    const scroll = canvas.getByRole("region", {
      name: "横スクロールカレンダー",
    });
    const today = within(scroll).getByRole("button", {
      name: /^2026年3月6日:/u,
    });
    ...
    await expect(march.querySelectorAll("[data-calendar-date]")).toHaveLength(
      31
    );
    await waitFor(
      async () =>
        await expect(
          Math.abs(
            march.getBoundingClientRect().left -
    ...
    await expect(
      document.getByText("この日のリリース記録はありません。")
    ).toBeVisible();
    await userEvent.click(
      canvas.getByRole("button", { name: /直前のリリース/u })
    );
    const releaseDay = within(scroll).getByRole("button", {
      name: /^2026年3月5日:/u,
    });
    await userEvent.hover(releaseDay);
    await expect(document.getByRole("tooltip")).toHaveTextContent(
      "Gemini Flash Next / GPT Next"
    );
    ...
    await expect(
      canvas.getByRole("button", { name: "月へ移動" })
    ).toHaveTextContent("2026年2月");
    await chooseCalendarDate(canvasElement, "2025-12-01");
    await userEvent.click(
      within(scroll).getByRole("button", { name: /^2025年12月31日:/u })
    );
    await expect(
      document.getByRole("article", { name: "Claude Opus Previous" })
    ).toBeVisible();
    await userEvent.click(canvas.getByRole("button", { name: "次の月" }));
    await expect(
      canvas.getByRole("button", { name: "月へ移動" })
    ).toHaveTextContent("2026年1月");
    await userEvent.click(canvas.getByRole("button", { name: "今日に戻る" }));
    await expect(
      canvas.getByRole("button", { name: "月へ移動" })
    ).toHaveTextContent("2026年3月");
    await waitFor(
      async () =>
        await expect(
          Math.abs(
...
export const Filtering: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      document.getByRole("article", { name: "Claude Opus Next" })
    ).toHaveTextContent("Claude Opus Previous");
    await userEvent.type(search, " missing");
    await expect(
      canvas.getByText("条件に一致するリリースがありません。")
    ).toBeVisible();
    await userEvent.click(
    ...
    await expect(canvas.getByRole("status")).toHaveTextContent("8 件中 1 件");
    await userEvent.click(
      canvas.getByRole("button", { name: /GPT Image · 2025年12月20日/u })
    );
    await expect(
...
export const Intervals: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(document.queryByRole("dialog")).not.toBeInTheDocument();
    await expect(
      canvas.getByRole("button", { name: /^2026年3月6日:/u })
    ).toHaveAttribute("aria-pressed", "true");
  },
};
...
export const UndatedAndEmptyMonth: Story = {
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(
      canvas.getByRole("button", { name: /^2026年4月1日:/u })
    );
    await expect(
    ...
    await expect(
      canvas.getByText("日付が未詳の 1 件は、一覧で確認できます。")
    ).toBeVisible();
    await userEvent.click(canvas.getByRole("button", { name: "一覧で見る" }));
    await expect(
      canvas.getByRole("article", { name: "Unknown date" })
    ).toBeVisible();
    await userEvent.click(
      canvas.getByRole("button", { name: "絞り込みを解除" })
    );
...
export const English: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.click(
      canvas.getByRole("button", { name: /GPT · March 5, 2026/u })
    );
    await expect(
      document.getByRole("article", { name: "GPT Next" })
    ).toHaveTextContent("After 63 days");
    await userEvent.click(
      document.getByRole("button", { name: "Close release details" })
    );
    await expect(document.queryByRole("dialog")).not.toBeInTheDocument();
    await userEvent.click(canvas.getByRole("button", { name: "Calendar" }));
    await expect(
      canvas.getByRole("heading", { name: "Monthly release calendar" })
    ).toBeVisible();
    await expect(
      canvas.getByRole("table", { name: "March 2026 calendar" })
    ).toBeVisible();
    await expect(
      canvas.getByRole("button", { name: /^March 6, 2026:/u })
    ).toHaveAttribute("aria-current", "date");
    await userEvent.click(
      canvas.getByRole("button", { name: "Jump to month" })
    );
    const picker = within(
      document.getByRole("dialog", { name: "Choose a date to navigate to" })
    );
    await userEvent.selectOptions(
    ...
    await userEvent.selectOptions(
      picker.getByRole("combobox", { name: "Choose the Month" }),
      "1"
    );
    await userEvent.click(
      picker.getByRole("button", { name: /February 29.*2024/u })
    );
    await expect(
      canvas.getByRole("button", { name: /^February 29, 2024:/u })
    ).toHaveAttribute("aria-pressed", "true");
    await expect(
      canvas.getByRole("button", { name: "Jump to month" })
...
export const NearbyReleases: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await userEvent.unhover(first);
    await expect(
      document.getByRole("article", { name: "Model A" })
    ).toBeVisible();
    ...
    await expect(
      document.getByRole("article", { name: "Model B" })
    ).toHaveTextContent("前回から 1 日");
    ...
    await userEvent.tab();
    await expect(
      document.getByRole("button", { name: "リリースの詳細を閉じる" })
    ).toHaveFocus();
    await userEvent.tab();
    await expect(
      document.getByRole("link", { name: "Official announcement" })
    ).toHaveFocus();
    await userEvent.keyboard("{Escape}");
    await expect(second).toHaveFocus();
    await userEvent.click(second);
...
export const FullHistory: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const document = within(canvasElement.ownerDocument.body);
    const chartElement = canvas.getByRole("region", {
      name: "リリース間隔の比較チャート",
    });
    const chart = within(chartElement);
    const pointCount = chart.getAllByRole("button").length;
    await expect(
      chart.getByRole("button", { name: /GPT · 2019年11月5日/u })
    ).toBeInTheDocument();
    await expect(
      chart.getByRole("button", { name: /GPT · 2026年3月5日/u })
    ).toBeInTheDocument();
    await expect(
      canvas.queryByRole("combobox", { name: "表示年" })
    ).not.toBeInTheDocument();
    ...
    await expect(
      document.getByRole("article", { name: "Early GPT" })
    ).toBeVisible();
    ...
    await userEvent.selectOptions(
      canvas.getByRole("combobox", { name: "年へ移動" }),
      "2021"
    );
    ...
    await waitFor(
      async () =>
        await expect(chartElement.scrollWidth).toBeGreaterThan(originalWidth)
    );
    await userEvent.click(canvas.getByRole("button", { name: "全期間を表示" }));
    await waitFor(
      async () =>
        await expect(chartElement.scrollWidth).toBeLessThanOrEqual(
          chartElement.clientWidth + 1
        )
    );
    await expect(chart.getAllByRole("button")).toHaveLength(pointCount);
    await userEvent.click(canvas.getByRole("button", { name: "Anthropic" }));
    await expect(
      chart.getByText("2019年", { exact: true })
    ).toBeInTheDocument();
    await expect(
      chart.getByRole("button", { name: /Claude Opus · 2025年12月31日/u })
    ).toBeInTheDocument();
...
export const DatePickerKeyboardAndCancel: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(
      reopened.getByRole("combobox", { name: "月を選択" })
    ).toHaveValue("2");
    await waitFor(
      async () =>
        await expect(
          reopened.getByRole("button", { name: /2026年3月6日/u })
        ).toHaveFocus()
    );
    await userEvent.keyboard("{ArrowRight}{Enter}");
    await expect(
      canvas.getByRole("button", { name: /^2026年3月7日:/u })
    ).toHaveAttribute("aria-pressed", "true");
    await waitFor(async () => await expect(trigger).toHaveFocus());
  },
};
...
export const CalendarAcrossYears: Story = {
  play: async ({ canvasElement }) => {
    ...
    const leapDay = within(scroll).getByRole("button", {
      name: /^2024年2月29日:/u,
    });
    ...
    await waitFor(
      async () =>
        await expect(
          within(scroll).getByRole("button", { name: /^2024年3月1日:/u })
        ).toHaveFocus()
    );
    ...
    scroll.scrollLeft +=
      requireValue(
        scroll.querySelector('[data-calendar-month="2024-02"]')
      ).getBoundingClientRect().width + 16;
    await waitFor(
      async () => await expect(monthPicker).toHaveTextContent("2024年3月")
    );
    await expect(monthPicker).toHaveFocus();
    await expect(
      scroll.querySelectorAll("[data-calendar-month]").length
    ).toBeLessThan(7);
    ...
    await waitFor(
      async () => await expect(monthPicker).toHaveTextContent("2026年2月")
    );
    ...
    scroll.scrollLeft = 0;
    await waitFor(
      async () => await expect(monthPicker).toHaveTextContent("2009年1月")
    );
    await waitFor(
      async () => await expect(scroll.scrollLeft).toBeGreaterThan(0)
    );
  },
};
...
export const ListDetailsAndOrder: Story = {
  play: async ({ canvasElement }) => {
    ...
    const list = within(
      canvas.getByRole("region", { name: "全期間のリリース一覧" })
    );
    ...
    await expect(
      gpt.queryByRole("link", { name: "Official announcement" })
    ).not.toBeVisible();
    await userEvent.click(gpt.getByText("リリースの詳細"));
    await expect(
      gpt.getByRole("link", { name: "Official announcement" })
    ).toHaveAttribute("href", "https://example.com/release");
    ...
    await expect(list.getAllByRole("article")[0]).toHaveAccessibleName(
      "GPT Image"
    );
    await expect(list.getAllByRole("article").at(-1)).toHaveAccessibleName(
      "Unknown date"
    );
  },
};
...
export const Empty: Story = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await expect(
      canvas.getByText("リリースデータがありません。")
    ).toBeVisible();

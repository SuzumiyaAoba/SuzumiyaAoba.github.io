...
export function AssetFormationSimulator({
  locale,
}: AssetFormationSimulatorProps) {
  const { t, numberFormatter, formatYenWithMan, formatYears } =
    useSimulatorFormatters(locale);
  const {
    scenarioList,
    scenarioData,
    selectedScenario,
    ...
    <main className="site-main flex flex-col pt-5 sm:pt-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <label className="flex flex-col gap-2">
          <span className="text-sm">
            {t("積立て期間（年）", "Contribution period (years)")}
          </span>
          <input
            type="number"
            inputMode="numeric"
      ...
      <div className="overflow-x-auto">
        <div className="mb-3 flex items-center gap-2 text-sm">
          <label
            htmlFor="visible-scenario-select"

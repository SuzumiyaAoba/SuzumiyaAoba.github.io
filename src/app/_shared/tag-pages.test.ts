...
describe("タグの静的ルート・メタデータ・サイトマップ", () => {
  it("英語版だけのタグも両言語のサイトマップに含め、翻訳の最新日付を使う", async () => {
    ...
    const tagPages = (await sitemap()).filter((page) => page.url.includes("/tags/"));
  ...
  it("記事内の重複を数えず、フォールバックを含めた件数をメタデータに反映する", async () => {
    ...
    expect((await buildTagPageMetadata(translatedProps, "ja")).description).toContain("0件");
    expect((await buildTagPageMetadata(translatedProps, "en")).description).toContain("(1)");
  });
});

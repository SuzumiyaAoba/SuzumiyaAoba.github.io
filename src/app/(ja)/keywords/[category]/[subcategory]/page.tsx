...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: item?.name ?? oldKeyword?.name ?? "Keywords",
    description: item?.description ?? oldKeyword?.effect,
    alternates: buildLocaleAlternates(canonicalPath, "ja"),
  };
}
...
export default async function Page({ params }: Props) {
  ...
  if (
    ...
    redirect(`/keywords/${legacy.category.id}/${category}/${subcategory}/`);
  }
  return (
    <KeywordSubcategoryPage
      locale="ja"

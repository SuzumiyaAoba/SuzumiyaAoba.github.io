...
await Promise.all(targets.map(async (target) => await patchFile(target)));

const googleVerificationPath = "out/google19f820ba5c9c10b8.html";
try {
  const raw = await fs.readFile(googleVerificationPath, "utf-8");
  if (!raw.includes("<html")) {
    const token = raw.trim();
    const wrapped = `<!doctype html><html lang="ja"><head><meta charset="utf-8"></head><body>${token}</body></html>`;
    await fs.writeFile(googleVerificationPath, wrapped, "utf-8");
  }
} catch {
  // ignore missing file
}
